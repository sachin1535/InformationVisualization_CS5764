...
for folder in foldersList:
	...
	for file in os.listdir(path+folder):
		if file.endswith(".dwg"):
			...
			try:
				...
				if not folder in datasets_exist:
				...
				for featureCls in cadBlockRefNames:
					arcpy.Select_analysis(path2features+"\\" +featureCls ,"temp1.shp", "\"Layer\" = 'E-COMM-WIRELESS'" or "\"PORTAL\" <>  ' '" )
					#infoNLWAP = arcpy.da.TableToNumPyArray("temp1.shp","PORTAL")
					#print(infoNLWAP)				
					arcpy.Select_analysis(path2features+"\\" + "Annotation","temp2.shp", "\"Layer\" = 'E-COMM-WIRELESS'" or "\"Text\" <>  ' '")
	...
	Combo = list();
	waplist = (arcpy.da.TableToNumPyArray(folder+"WAP_Points.shp","WAPID")).tolist();
	for ele in waplist:
		for name in ele:
			parts = name[0].split(',');
	...
	for ele in range(len(waplist)):
		features[ele].WAPID = Combo[ele];
